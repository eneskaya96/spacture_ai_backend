...
class DetectedImagesService:
    ...
    def send_detected_images(self):
        detected_person = self.detected_person_service.create_detected_person(self.detection_count)

        self.socketio.emit('new_detection', {'data': [detected_person] }, namespace='/')

        self.detection_count = 1 if self.detection_count == 3 else self.detection_count + 1
